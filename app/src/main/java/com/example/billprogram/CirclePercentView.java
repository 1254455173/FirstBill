...
public class CirclePercentView extends View {
    ...
    public CirclePercentView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mSmallColor = a.getColor(R.styleable.CirclePercentView_smallColor,0x88ffcc66);
        mBigColor = a.getColor(R.styleable.CirclePercentView_bigColor,0x00000000);
    ...
    //内部设置百分比 用于动画效果
    private void setCurPercent(int percent) {       // 异步刷新操作

        mPercent = percent; // 百分比

        new Thread(new Runnable() {
            @Override
            public void run() { // Runnable 接口
                int sleepTime = 20; // 异步刷新时间
                for(int i =0;i<mPercent;i++){
                    ...
                    mCurPercent = i;
                    CirclePercentView.this.postInvalidate();    // 线程刷新
                }
                }
